...
class update extends JFrame implements ActionListener,ItemListener{
	...
	JButton b1,b2,b5,b6,b7, b8, b9,b10,b11,add,remove,update,return_car,search,GO;
    ...
    update()
	{
		...
		form.setBounds(420,200,800,550);
		background.add(form);
		form_title=new JLabel("UPDATE CAR");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==b11)
		{
			 try
			{
			  prstm=cn.prepareStatement("select * from car where CAR_NO='"+t1.getText()+"'");
				...
				if(rs.next())
				{
					if(t1.getText().length()>0 && t2.getText().length()>0 && t7.getText().length()>0 && t8.getText().length()>0)
					{
						...
						try{ 
						sql="Update car set car_price="+t2.getText()+",fuel_type='"+t3.getSelectedItem()+"',pd_price="+t4.getSelectedItem()+",seats='"+t5.getSelectedItem()+" ' ,car_brand='"+t6.getSelectedItem()+"',car_color='"+t7.getText()+"' ,car_adv="+t8.getText()+" where car_no='"+t1.getText()+"'";
						prstm=cn.prepareStatement(sql);
						prstm.execute();
						JOptionPane.showMessageDialog(null,"CAR RECORD EDIT SUCESSFULLY");
						}
						catch(Exception e1)
						{
			 ...
			 prstm=cn.prepareStatement("select * from car where CAR_NO='"+t1.getText()+"'");
				...
				if(rs.next())
				{
					...
					while(rs.next())
					{
						...
						t5.setSelectedItem(rs.getString(5));
						t6.setSelectedItem(rs.getString(6));
