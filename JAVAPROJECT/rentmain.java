...
class rentmain extends JFrame implements ActionListener {

     
	
	JButton b1,b2,b5, b6, b7, b8, b9,b10,b11,add,remove,update,return_car,search,searchPhone;
	JLabel background,name,form_title,l1,l2,l3,l4,l5,l6,l7,l8,l9;
    ...
    rentmain(String carNo) {
		...
		b11=new JButton("RENT");
		searchPhone=new JButton("search");
		searchPhone.setBounds(570,320,200,30);
		searchPhone.addActionListener(this);
		
		 Random rmd=new Random();
		 int custID=rmd.nextInt(100);
		t2.setText(""+custID);
		
		 
		
		Random rmd1=new Random();
		int rentId=rmd1.nextInt(100);
		t7.setText(""+rentId);
		...
		b10.setBounds(220,555,130,40);
		b11.setBounds(430,555,130,40);
		...
		calb.setBounds(360,470,200,30); 

		l9.setBounds(220,520,140,30);
		calb2.setBounds(360,520,200,30); 
		...
		form.add(l8);  form.add(calb);
		form.add(l9); form.add(calb2);
		 		 
		form.add(b10);
		form.add(b11);
		form.add(searchPhone);
		 
		Font f2=new Font("serif",Font.BOLD,30);
		 form_title=new JLabel("RENT CAR");
		form_title.setLayout(null);
		form.setBackground(new Color(0,0,0,50));
		form.setBounds(390,180,800,750);
		form.setBackground(Color.ORANGE);
		background.add(form);
		 
		form_title.setBounds(300,0,400,100);
		form.add(form_title);
		form_title.setFont(f2);
	 ...
	 if(e.getSource()==b11)
		{
						...
						try
						{
							...
							if( t1.getText().length()>0 && t2.getText().length()>0 && t3.getText().length()>0 && t4.getText().length()>0 && t5.getText().length()>0 && t6.getText().length()>0  && t7.getText().length()>0 && calb.getText().length()>0 && calb2.getText().length()>0)
							{
								...
								JOptionPane.showMessageDialog(null, "Please enter a valid 10-digit phone number", "Error", JOptionPane.ERROR_MESSAGE);
								return;
								}
								...
								java.util.Date rentDate = dateFormat.parse(calb.getText()); // Specify java.util.Date
								java.util.Date returnDate = dateFormat.parse(calb2.getText()); // Specify java.util.Date


            // Check if return date is after rent date
								if (returnDate.before(rentDate)) {
									JOptionPane.showMessageDialog(null, "Return date should be after rent date", "Error", JOptionPane.ERROR_MESSAGE);
									return;
								}
							...
							//sql="insert into car values('"+t1.getText()+"',"+t2.getText()+",'"+t3.getSelectedItem()+"',"+t4.getSelectedItem()+","+t5.getSelectedItem()+",'"+t6.getSelectedItem()+"','"+t7.getText()+"',"+t8.getText()+")";					 
								...
								sql="insert into RENT values('"+t1.getText()+"',"+t2.getText()+",'"+t3.getText()+"','"+t4.getText()+"','"+t5.getText()+"','"+t6.getText()+"','"+t7.getText()+"','"+calb.getText()+"','"+calb2.getText()+"')";					 
								prstm=cn.prepareStatement(sql);
								prstm.execute();
								JOptionPane.showMessageDialog(null,"Car give on rent sucessfully");
		...
		if(e.getSource()==searchPhone)
		{
			try {
				prstm = cn.prepareStatement("select rent.CUS_NAME, rent.ADDRESS,rent.CUS_EMAIL from rent where rent.CUS_PNUM='" + t5.getText() + "'");
