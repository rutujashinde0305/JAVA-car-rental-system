...
class add2 extends JFrame implements ActionListener
 {
		...
		JButton b1,b2,b5, b6, b7, b8, b9,b10,b11,add,remove,update,return_car,search;
		JLabel background,name,form_title,l1,l2,l3,l4,l5,l6,l7,l8;
		TextField t1,t2, t7,t8;
		JComboBox<String> t3, t6;
		JComboBox<Integer> t4, t5;
	...
	add2()
	{
		...
		l1=new JLabel("Car NO");
		l2=new JLabel("CAR PRICE");
		l3=new JLabel("FUEL TYPE");
		l4=new JLabel("PD PRICE");
		l5=new JLabel("SEATS INT");
		l6=new JLabel("CAR BRAND");
		l7=new JLabel(" CAR COLOR");
		l8=new JLabel("ADVANCE AMOUNT");
		
		t1=new TextField(20);
		t2=new TextField(20);
		t7=new TextField(20);
		t8=new TextField(20);
		t3 = new JComboBox<>(new String[]{"Petrol", "Diesel", "CNG", "Electric Batteries"});
        t4 = new JComboBox<>(new Integer[]{1000, 2000, 3000, 4000});
        t5 = new JComboBox<>(new Integer[]{4, 5, 6, 7});
        t6 = new JComboBox<>(new String[]{"BMW", "HYUNDAI", "AUDI", "FERRARI"});
		...
		b10.setBounds(220,510,130,40);
		 b11.setBounds(430,510,130,40);
		 
	
	
		l1.setBounds(220,120,140,30);
		t1.setBounds(360,120,200,30);
		l2.setBounds(220,170,140,30);
		t2.setBounds(360,170,200,30);
		l3.setBounds(220,220,140,30);
		t3.setBounds(360,220,200,30);
		l4.setBounds(220,270,140,30);
		t4.setBounds(360,270,200,30);
		l5.setBounds(220,320,140,30);
		t5.setBounds(360,320,200,30);
		l6.setBounds(220,370,140,30);
		t6.setBounds(360,370,200,30);
		l7.setBounds(220,420,140,30);
		t7.setBounds(360,420,200,30); 
		l8.setBounds(220,470,140,30);
		t8.setBounds(360,470,200,30);
		...
		form.setBackground(new Color(0,0,0,10));
		form.setBackground(Color.ORANGE);
		form.setBounds(430,200,750,550);
		background.add(form);
		form_title=new JLabel("ADD NEW CAR");
		...
		public void display()
			...
			ResultSet rs;
			JTable table;
			String colHeads[]={"CAR_NO","CAR_PRICE","FUEL_TYPE","PD_PRICE","SEATS","CAR_BRAND","CAR_COLOR","CAR_ADV"};
			String Data[][];
			int cnt=0,i=0;
			...
			try
			{
				...
				rs=stm.executeQuery("select *from CAR order by CAR_NO");
				for(int x=0;x<1;x++)
				{
					rs.beforeFirst();
					while(rs.next())
				...
				jsp.setBounds(1000,370,500,400);
				add(jsp);
	...
	public void actionPerformed(ActionEvent e)
	{
		 ...
		 if (e.getSource() == b11) {
            String fuelType = (String) t3.getSelectedItem();
            int pdPrice = (int) t4.getSelectedItem(); 
            int seats = (int) t5.getSelectedItem(); 
            String carBrand = (String) t6.getSelectedItem();

            
        
	 
				String fisrtTwoLetter=t1.getText().substring(0,2);
				String  twoDigit=t1.getText().substring(2,4);
				String  nextTwoLetter=t1.getText().substring(4,6);
				String lastFourDigit=t1.getText().substring(6);
			 
				if( t1.getText().length()>0 && t2.getText().length()>0  && t7.getText().length()>0 && t8.getText().length()>0)
				{
					if(!fisrtTwoLetter.matches("[A-Z]+"))
					{
						JOptionPane.showMessageDialog(this,"car number must star with two letters");
						return;
					}
					if(!twoDigit.matches("\\d{2}"))
					{
						JOptionPane.showMessageDialog(this,"car number  must contain two digit after first two lettter.");
						return;
					}
					if(!nextTwoLetter.matches("[A-Z]+"))
					{
						JOptionPane.showMessageDialog(this,"car number  must contain two letter after two digit.");
						return;
					}
					if(!lastFourDigit.matches("\\d{4}"))
					{
						JOptionPane.showMessageDialog(this,"car number must end with four digit");
						return;
					}
					
					String prizeText = t2.getText();
					if (!prizeText.matches("\\d+")) {
					JOptionPane.showMessageDialog(this, "Please enter an integer value for the car prize.");
					return;
					}
					//int prize = Integer.parseInt(prizeText);
				
				
					String color = t7.getText();
					if (!color.matches("[a-zA-Z]+")) {
					JOptionPane.showMessageDialog(this, "Please enter only string values for car color.");
					return;
					}


					String advance = t8.getText();
					if (!advance.matches("\\d+")) {
					JOptionPane.showMessageDialog(this, "Please enter an integer value for advance amount.");
					return;
					}
					//int amount = Integer.parseInt(advance); 
						...
						try
						{
							sql="insert into car values('"+t1.getText()+"',"+t2.getText()+",'"+t3.getSelectedItem()+"',"+t4.getSelectedItem()+","+t5.getSelectedItem()+",'"+t6.getSelectedItem()+"','"+t7.getText()+"',"+t8.getText()+")";					 
						...
						{
							JOptionPane.showMessageDialog(null, "Failed to connect to the database: " + e1.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
							e1.printStackTrace();  
						}
