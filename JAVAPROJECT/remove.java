...
class remove extends JFrame implements ActionListener {
    ...
    remove() {
		...
		background.add(form);
		form_title=new JLabel("REMOVE CAR");
        ...
        background.add(heading);
		 ...
		 Font f4=new Font("serif",Font.BOLD,60);
			  ...
			  prstm=cn.prepareStatement("select * from car where CAR_NO='"+t1.getText()+"'");
				...
				if(rs.next())
				{
					...
					if(t1.getText().length()>0 && t2.getText().length()>0 && t3.getText().length()>0 && t4.getText().length()>0 && t5.getText().length()>0 && t6.getText().length()>0 && t7.getText().length()>0&& t8.getText().length()>0)
					{
						...
						try{
							sql="delete from car where car_no='"+t1.getText()+"'"; 
							prstm=cn.prepareStatement(sql);
							prstm.execute();
							JOptionPane.showMessageDialog(null,"CAR REMOVE SUCESSFULLY");
							}
						catch(Exception e1)
						{
